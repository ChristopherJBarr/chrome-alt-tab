...
async function render() {
	...
	previews.disabled = !session.connected || !["0.4.0", "0.5.0"].includes(session.hostVersion);
	document.getElementById("preview-status").textContent = enabled
		...
		: (session.connected && !["0.4.0", "0.5.0"].includes(session.hostVersion) ? "Helper update required. Reconnect after registering the new build." : "Off. Tabs use a placeholder until you enable previews and visit them.");
